...
export default function RandomUnderline({ children, className, color }: Props) {
  ...
  const nextIndex = useMemo(() => {
    ...
    if (lastIndex !== null && n > 1) {
      while (idx === lastIndex) idx = Math.floor(Math.random() * n);
    }
    return idx;
  }, [lastIndex]);

  function drawIn(path: SVGPathElement, duration = 500) {
  ...
  function drawOut(path: SVGPathElement, duration = 500, onDone: () => void) {
    const total = path.getTotalLength();
    path.style.strokeDasharray = `${total}`;
    ...
    const step = (t: number) => {
      const p = Math.min(1, (t - start) / duration);
      path.style.strokeDashoffset = `${total * p}`;
      if (p < 1) {
        animRef.current = requestAnimationFrame(step);
      } else {
        animRef.current = null;
  ...
  function handleEnter() {
    ...
    boxRef.current.innerHTML = SVG_VARIANTS[nextIndex];
  ...
  function handleLeave() {
    if (!boxRef.current) return;
    const path = boxRef.current.querySelector("path") as SVGPathElement | null;
    if (!path) return;
    if (animRef.current) cancelAnimationFrame(animRef.current);
    drawOut(path, 500, () => {
  ...
  return (
    ...
    >
      ...
      <span className={styles.box} aria-hidden="true" ref={boxRef} />
    </span>
  );
}